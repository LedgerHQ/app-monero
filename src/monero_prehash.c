...
#define aH AKout
    ...
    if (G_monero_vstate.tx_sig_mode == TRANSACTION_CREATE_REAL) {
        ...
        // A short amount carries only 8 meaningful bytes, and the value shown to
        // the user is read from those low bytes. Reject a non-canonical encoding
        // so the displayed amount matches what the commitment binds.
        if ((G_monero_vstate.options & 0x03) == 0x02) {
