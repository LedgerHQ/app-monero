...
void monero_clear_words(void) {
    monero_nvm_write((void *) N_monero_pstate->words_list,
                     NULL,
 ...
 */
static int monero_set_word(unsigned int n,
                           unsigned int idx,
        ...
        case 1:
            ...
            if (G_monero_vstate.io_p2) {
                ...
                monero_nvm_write((void *) N_monero_pstate->words[24],
                                 (void *) N_monero_pstate->words[w_start],
                                 WORDS_MAX_LENGTH);
...
int monero_apdu_display_address() {
    ...
    error = ui_menu_any_pubaddr_display(0,
                                        C,
                                        D,
                                        (minor | major) ? 1 : 0,
...
int monero_apdu_verify_key() {
    ...
    switch (G_monero_vstate.io_p1) {
        case 0: {
            ...
            err = monero_secret_key_to_public_key(computed_pub,
                                                  priv,
                                                  sizeof(computed_pub),
                                                  sizeof(priv));
            explicit_bzero(priv, sizeof(priv));
...
int monero_apdu_derive_public_key(/*const crypto::key_derivation &derivation, const std::size_t output_index, const crypto::public_key &pub, public_key &derived_pub*/) {
    ...
    // pub
    int error = monero_derive_public_key(drvpub,
                                         derivation,
                                         output_index,
                                         pub,
                                         sizeof(drvpub),
...
int monero_apdu_derive_secret_key(/*const crypto::key_derivation &derivation, const std::size_t output_index, const crypto::secret_key &sec, secret_key &derived_sec*/){
    ...
    // pub
    err = monero_derive_secret_key(drvsec,
                                   derivation,
                                   output_index,
                                   sec,
                                   sizeof(drvsec),
...
int monero_apdu_derive_subaddress_public_key(/*const crypto::public_key &pub, const crypto::key_derivation &derivation, const std::size_t output_index, public_key &derived_pub*/) {
    ...
    // pub
    err = monero_derive_subaddress_public_key(sub_pub,
                                              pub,
                                              derivation,
                                              output_index,
                                              sizeof(sub_pub),
...
int monero_apdu_get_subaddress_secret_key(/*const crypto::secret_key& sec, const cryptonote::subaddress_index& index, crypto::secret_key &sub_sec*/) {
    ...
    err = monero_get_subaddress_secret_key(sub_sec,
                                           sec,
                                           index,
                                           sizeof(sub_sec),
                                           sizeof(sec),
...
#define out_eph_public_key additional_txkey_sec
    ...
    if (need_additional_txkeys) {
        if (is_subaddress) {
            err = monero_ecmul_k(additional_txkey_pub,
                                 Bout,
                                 additional_txkey_sec,
            ...
            err = monero_ecmul_G(additional_txkey_pub,
                                 additional_txkey_sec,
                                 sizeof(additional_txkey_pub),
                                 sizeof(additional_txkey_sec));
            if (err) {
    ...
    // derivation
    if (is_change) {
        err = monero_generate_key_derivation(derivation,
                                             txkey_pub,
                                             G_monero_vstate.a,
                                             sizeof(derivation),
                                             KEY_SIZE,
    ...
    err = monero_derivation_to_scalar(amount_key,
                                      derivation,
                                      output_index,
                                      sizeof(amount_key),
                                      sizeof(derivation));
    ...
    err = monero_derive_public_key(out_eph_public_key,
                                   derivation,
                                   output_index,
                                   Bout,
                                   sizeof(out_eph_public_key),
                                   sizeof(derivation),
                                   KEY_SIZE);
    if (err) {
        explicit_bzero(tx_key, sizeof(tx_key));
