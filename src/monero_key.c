 ...
 */
static int monero_set_word(unsigned int n, unsigned int idx, unsigned int w_start,
                            unsigned char *word_list, int len) {
    while (w_start < idx) {
        len -= 1 + word_list[0];
        if (len < 0) {
            monero_clear_words();
            return SW_WRONG_DATA + 1;
        }
            ...
            for (int i = 0; i < 8; i++) {
                ...
                for (int wi = 0; wi < 3; wi++) {
                    if ((wc[wi] >= w_start) && (wc[wi] < w_end)) {
                        err = monero_set_word(i * 3 + wi, wc[wi], w_start,
...
int monero_apdu_display_address() {
    ...
    if (minor | major) {
        ...
        memcpy(C, G_monero_vstate.A, 32);
        memcpy(D, G_monero_vstate.B, 32);
    }
...
int monero_apdu_put_key() {
    ...
    monero_ecmul_G(raw, sec, sizeof(raw), sizeof(sec));
...
int monero_apdu_verify_key() {
    ...
    switch (G_monero_vstate.io_p1) {
        case 0:
            monero_secret_key_to_public_key(computed_pub, priv, sizeof(computed_pub), sizeof(priv));
            break;
        case 1:
...
int monero_apdu_get_chacha8_prekey(/*char  *prekey*/) {
    ...
    monero_io_discard(0);
    memcpy(abt, G_monero_vstate.a, 32);
    memcpy(abt + 32, G_monero_vstate.b, 32);
...
int monero_apdu_sc_add(/*unsigned char *r, unsigned char *s1, unsigned char *s2*/) {
    ...
    int err;
    // fetch
    err = monero_io_fetch_decrypt(s1, 32, TYPE_SCALAR);
    if (err) {
        return err;
    }
    err = monero_io_fetch_decrypt(s2, 32, TYPE_SCALAR);
    if (err) {
        return err;
    }
    monero_io_discard(0);
    ...
    monero_addm(r, s1, s2, sizeof(r), sizeof(s1), sizeof(s2));
...
int monero_apdu_scal_mul_key(/*const rct::key &pub, const rct::key &sec, rct::key mulkey*/) {
    ...
    monero_ecmul_k(r, pub, sec, sizeof(r), sizeof(pub), sizeof(sec));
...
int monero_apdu_scal_mul_base(/*const rct::key &sec, rct::key mulkey*/) {
    ...
    int err;
    // fetch
    err = monero_io_fetch_decrypt(sec, 32, TYPE_SCALAR);
    if (err) {
        return err;
    }
    monero_io_discard(0);

    monero_ecmul_G(r, sec, sizeof(r), sizeof(sec));
    monero_io_insert(r, 32);
    return SW_OK;
}
...
int monero_apdu_generate_keypair(/*crypto::public_key &pub, crypto::secret_key &sec*/) {
    ...
    monero_io_discard(0);
    monero_generate_keypair(pub, sec, sizeof(pub), sizeof(sec));
...
int monero_apdu_secret_key_to_public_key(
    ...
    unsigned char pub[32];
    // fetch
    int err = monero_io_fetch_decrypt(sec, 32, TYPE_SCALAR);
    if (err) {
        return err;
    }
    monero_io_discard(0);
    // pub
    monero_ecmul_G(pub, sec, sizeof(pub), sizeof(sec));
    // pub key
    monero_io_insert(pub, 32);
    return SW_OK;
}
...
int monero_apdu_derivation_to_scalar(
    ...
    // fetch
    int err = monero_io_fetch_decrypt(derivation, 32, TYPE_DERIVATION);
    if (err) {
        return err;
    }
...
int monero_apdu_derive_public_key(/*const crypto::key_derivation &derivation, const std::size_t output_index, const crypto::public_key &pub, public_key &derived_pub*/) {
    ...
    // pub
    int error = monero_derive_public_key(drvpub, derivation, output_index, pub, sizeof(drvpub), sizeof(derivation), sizeof(pub));
    if (error) {
        return error;
    }
...
int monero_apdu_derive_secret_key(/*const crypto::key_derivation &derivation, const std::size_t output_index, const crypto::secret_key &sec, secret_key &derived_sec*/){
    ...
    // fetch
    err = monero_io_fetch_decrypt(derivation, 32, TYPE_DERIVATION);
    if (err) {
        return err;
    }
    output_index = monero_io_fetch_u32();
    ...
    // pub
    err = monero_derive_secret_key(drvsec, derivation, output_index, sec, sizeof(drvsec), sizeof(derivation), sizeof(sec));
    if (err) {
        return err;
    }

    // sec key
    memcpy(G_monero_vstate.last_derive_secret_key, drvsec, 32);
...
int monero_apdu_derive_subaddress_public_key(/*const crypto::public_key &pub, const crypto::key_derivation &derivation, const std::size_t output_index, public_key &derived_pub*/) {
    ...
    // pub
    monero_derive_subaddress_public_key(sub_pub, pub, derivation, output_index, sizeof(sub_pub), sizeof(pub), sizeof(derivation));
    // pub key
...
int monero_apdu_get_subaddress_spend_public_key(
    ...
    // pub
    monero_get_subaddress_spend_public_key(D, index, sizeof(D), sizeof(index));
...
int monero_apdu_get_subaddress_secret_key(/*const crypto::secret_key& sec, const cryptonote::subaddress_index& index, crypto::secret_key &sub_sec*/) {
    ...
    monero_get_subaddress_secret_key(sub_sec, sec, index, sizeof(sub_sec), sizeof(sec), sizeof(index));

    memcpy(G_monero_vstate.last_get_subaddress_secret_key, sub_sec, 32);
...
#define out_eph_public_key additional_txkey_sec
    ...
    if (need_additional_txkeys) {
        if (is_subaddress) {
            monero_ecmul_k(additional_txkey_pub, Bout, additional_txkey_sec, sizeof(additional_txkey_pub), 32, sizeof(additional_txkey_sec));
        } else {
            monero_ecmul_G(additional_txkey_pub, additional_txkey_sec, sizeof(additional_txkey_pub), sizeof(additional_txkey_sec));
        }
    } else {
    ...
    // derivation
    if (is_change) {
        monero_generate_key_derivation(derivation, txkey_pub, G_monero_vstate.a, sizeof(derivation), 32, sizeof(G_monero_vstate.a));
    } else {
        monero_generate_key_derivation(
            derivation, Aout,
            (is_subaddress && need_additional_txkeys) ? additional_txkey_sec : tx_key,
            sizeof(derivation), 32, 32);
    }

    // compute amount key AKout (scalar1), version is always greater than 1
    monero_derivation_to_scalar(amount_key, derivation, output_index, sizeof(amount_key), sizeof(derivation));
    if (G_monero_vstate.tx_sig_mode == TRANSACTION_CREATE_REAL) {
        if (G_monero_vstate.io_protocol_version >= 2) {
    ...
    monero_derive_public_key(out_eph_public_key, derivation, output_index, Bout, sizeof(out_eph_public_key), sizeof(derivation), 32);
