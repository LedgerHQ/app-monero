...
void monero_io_clear() {
  os_memset(G_monero_vstate.work.io_buffer, 0 , MONERO_IO_BUFFER_LENGTH);
}
...
void monero_io_hole(unsigned int sz) {
  ...
  os_memmove(G_monero_vstate.work.io_buffer+G_monero_vstate.io_offset+sz,
             G_monero_vstate.work.io_buffer+G_monero_vstate.io_offset,
...
void monero_io_insert(unsigned char const *buff, unsigned int len) {
  monero_io_hole(len);
  os_memmove(G_monero_vstate.work.io_buffer+G_monero_vstate.io_offset, buff, len);
  G_monero_vstate.io_offset += len;
}

void monero_io_insert_u32(unsigned  int v32) {
  monero_io_hole(4);
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+0] = v32>>24;
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+1] = v32>>16;
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+2] = v32>>8;
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+3] = v32>>0;
...
void monero_io_insert_u24(unsigned  int v24) {
  monero_io_hole(3);
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+0] = v24>>16;
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+1] = v24>>8;
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+2] = v24>>0;
  G_monero_vstate.io_offset += 3;
}
void monero_io_insert_u16(unsigned  int v16) {
  monero_io_hole(2);
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+0] = v16>>8;
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+1] = v16>>0;
  G_monero_vstate.io_offset += 2;
}
void monero_io_insert_u8(unsigned int v8) {
  monero_io_hole(1);
  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+0] = v8;
...
void monero_io_fetch_buffer(unsigned char* buffer, unsigned int len) {
  os_memmove(buffer, G_monero_vstate.work.io_buffer+G_monero_vstate.io_offset, len);
  G_monero_vstate.io_offset += len;
}

unsigned int monero_io_fetch_u32() {
  unsigned int  v32;
  v32 =  ( (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+0] << 24) |
           (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+1] << 16) |
           (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+2] << 8) |
           (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+3] << 0) );
  G_monero_vstate.io_offset += 4;
  return v32;
}
...
unsigned int monero_io_fetch_u24() {
  unsigned int  v24;
  v24 =  ( (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+0] << 16) |
           (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+1] << 8) |
           (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+2] << 0) );
  G_monero_vstate.io_offset += 3;
  return v24;
}
...
unsigned int monero_io_fetch_u16() {
  unsigned int  v16;
  v16 =  ( (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+0] << 8) |
           (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+1] << 0) );
  G_monero_vstate.io_offset += 2;
  return v16;
}
...
unsigned int monero_io_fetch_u8() {
  unsigned int  v8;
  v8 = G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset] ;
  G_monero_vstate.io_offset += 1;
  return v8;
}

int monero_io_fetch_t(unsigned int *T) {
  *T = G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset];
  G_monero_vstate.io_offset++;
  if ((*T & 0x1F) == 0x1F) {
    *T = (*T << 8) | G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset];
    G_monero_vstate.io_offset++;
  }
  return 0;
}
...
int monero_io_fetch_l(unsigned int *L) {
  *L = G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset];

  if ((*L & 0x80) != 0) {
    *L &= 0x7F;
    if (*L == 1) {
      *L =  G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+1];
      G_monero_vstate.io_offset += 2;
    } else if (*L == 2) {
      *L =  (G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+1] << 8) | G_monero_vstate.work.io_buffer[G_monero_vstate.io_offset+2] ;
      G_monero_vstate.io_offset += 3 ;
    } else {
      *L = -1;
    }
...
int monero_io_fetch_nv(unsigned char* buffer, int len) {
  monero_nvm_write(buffer, G_monero_vstate.work.io_buffer+G_monero_vstate.io_offset, len);
  G_monero_vstate.io_offset += len;
  return len;
}
...
int monero_io_do(unsigned int io_flags) {
  ...
  if (io_flags & IO_ASYNCH_REPLY) {
    ...
    while(G_monero_vstate.io_length > MAX_OUT) {
      ...
      //send chunk
      tx =  MAX_OUT-2;
      os_memmove(G_io_apdu_buffer, G_monero_vstate.work.io_buffer+G_monero_vstate.io_offset, tx);
    ...
    os_memmove(G_io_apdu_buffer,  G_monero_vstate.work.io_buffer+G_monero_vstate.io_offset, G_monero_vstate.io_length);
  ...
  default:
    ...
    os_memmove(G_monero_vstate.work.io_buffer, G_io_apdu_buffer+5, G_monero_vstate.io_lc);
  ...
  in_chaining:
    ...
    os_memmove(G_monero_vstate.work.io_buffer+G_monero_vstate.io_length, G_io_apdu_buffer+5,  G_monero_vstate.io_lc);
    G_monero_vstate.io_length +=  G_monero_vstate.io_lc;
  }
