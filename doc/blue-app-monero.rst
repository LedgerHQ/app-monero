...
to the Host.
Optionally the secret transaction key may be returned encrypted by SPK to be used later. Moreover the secret transaction key is discarded by the token at the end
of the transaction and can not be retrieved if not saved by host. If the secret 
transaction key needs to be saved, the SPK is generated in a deterministic
way.
